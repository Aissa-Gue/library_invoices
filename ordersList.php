...
// Search query
$searchQry = "SELECT * FROM c_orders, a_clients WHERE (c_orders.client_id = a_clients.client_id) AND (order_id LIKE '%$order_id' AND last_name LIKE '%$last_name%' AND first_name LIKE '%$first_name%' AND type LIKE '%$type' AND YEAR(c_orders.creation_date) LIKE '%$creation_year') $orderBy";
                                    ...
                                    <div class="form-row mt-3">
                                        ...
                                        <div class="input-group-prepend">
                                            ...
                                            <select class="custom-select" name="orderBy" id="orderBy">
                                                ...
                                                <option value="ORDER BY `type`, 'order_id' ASC">نوع الفاتورة</option>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <?php if ($row['type'] == 1) echo 'بيع';
                                                elseif ($row['type'] == 2) echo 'معرض';
                                                elseif ($row['type'] == 3) echo 'إهداء'; ?>
                                    </td>
                                    <td class="text-center"><?php echo '% ' . $row['discount_percentage'] ?></td>

                                    <td class="text-center">
                                        <a class="btn btn-outline-danger"
                                            href="previewOrder.php?order_id=<?php echo $row['order_id'] ?>&client_id=<?php echo $row['client_id'] ?>&type=<?php echo $row['type'] ?>&discount_percentage=<?php echo $row['discount_percentage'] ?>&total_price=<?php echo $row['total_price'] ?>&creation_date=<?php echo $row['creation_date'] ?>&last_edit_date=<?php echo $row['last_edit_date'] ?>&last_name=<?php echo $row['last_name'] ?>&first_name=<?php echo $row['first_name'] ?>&father_name=<?php echo $row['father_name'] ?>">
